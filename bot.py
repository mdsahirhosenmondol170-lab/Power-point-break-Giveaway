...
async def send_user_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"Hello @{user.username} ðŸŽ‰\n"
        f"ðŸ†” User ID: {user.id}\n\n"
        "ðŸ“© To participate in the giveaway,\n"
        "ðŸ‘‰ Please ðŸ‘‡ Tap the button!"
    )

    buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("ðŸš€ Join Giveaway Now!", callback_data="join_gv")],
...
async def send_admin_start(update: Update):
    ...
    buttons = [
        [InlineKeyboardButton("ðŸ“œ Show Commands", callback_data="help")],
    ]

    await update.message.reply_text(
        f"ðŸ‘‹ Hello Admin @{user.username}!\nWelcome to your Giveaway Bot âœ…",
...
async def verificationlink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "ðŸ“© Send @channel usernames (max 5)\n"
        "One per message.\n"
...
async def join_gv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not DATA["giveaway_enabled"]:
        return await query.message.reply_text(
            "â›”ï¸ âŒ GIVEAWAY CLOSED âŒ â›”ï¸\n"
            "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"
            f"ðŸ“© Contact Admin:\nðŸ‘‰ @{ADMIN_USERNAME}\n"
            "ðŸ’« Please try another Giveaway!\n"
            "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”"
        )
    ...
    if uid in DATA["old_winners"]:
        return await query.message.reply_text(
            ...
            "ðŸ† Youâ€™ve already won this giveaway!\n\n"
            "âš–ï¸ To keep things fair, repeat participation isnâ€™t allowed.\n"
            "ðŸ™ Thank you for understanding!\n\n"
            f"ðŸ“© Admin Support: @{ADMIN_USERNAME}\n"
            "â”—â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”›"
        )
    ...
    for w in DATA["winners"]:
        if w["user_id"] == uid:
            return await query.message.reply_text(
                ...
                f"ðŸ“© For any concerns, please contact:\nðŸ‘‰ @{ADMIN_USERNAME}\n"
                "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”"
            )
    ...
    if len(DATA["winners"]) >= DATA["winner_limit"]:
        return await query.message.reply_text(
            "ðŸ˜” Oops! All winners are already selected!\n"
            "ðŸŽ‰ Thanks for joining!\n\n"
            "ðŸ€ Try again â€” more giveaways soon!\n"
            "ðŸ’™ Stay with Power Point Break!\n\n"
            f"ðŸ“ž For support:\nðŸ‘‰ @{ADMIN_USERNAME}\n"
            "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”"
        )
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                "You are one of the WINNERS of our Giveaway! ðŸ†\n\n"
                f"ðŸ“© Contact Admin to claim your reward:\nðŸ‘‰ @{ADMIN_USERNAME}\n\n"
                "ðŸ’™ Hosted by: Power Point Break"
            )
        )
    except:
        pass
    ...
    await query.message.reply_text(
        "ðŸŽ‰ You are SELECTED as Winner!\n"
        "Check your DM! ðŸ’Œ"
      )
...
async def run_auto_countdown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.edit_text(
        ...
        "ðŸŽ‰ Stay ready â€” Winners will be announced very soon!\n\n"
        "ðŸŽ™ Hosted by: Power Point Break"
    )
...
async def auto_pick_winners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        [
            ...
            InlineKeyboardButton("âŒ REJECT",  callback_data="auto_no"),
        ]
    ])
...
async def auto_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user):
        return await query.answer("âŒ Not allowed")

    txt = "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n"
    txt += "ðŸ† Power Point Break â€” Giveaway Winners\n"
    txt += "â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”\n\n"
...
async def auto_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text("âŒ Cancelled!")
...
async def countdown(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "ðŸ“© Send countdown message text\n\n"
        "Example:\nðŸ”¥ Giveaway starts soon!"
    )
